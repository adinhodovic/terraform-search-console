resource "cloudflare_record" "default" {
  ...
  name    = lookup(var.cloudflare_zones[count.index], "name")
  value   = lookup(var.cloudflare_zones[count.index], "value")
  type    = "TXT"
}
...
resource "aws_route53_record" "default" {
  ...
  records = lookup(var.aws_route53_domains[count.index], "value")
  type    = "TXT"
}
